...
TEST(mkp_test, mkp_test_004_avc)
{
	...
	//Insert an avc node
	trace_android_rvh_selinux_avc_insert((struct avc_node*)&node);
	//modify avc node
	node.ae.tclass=0xAA;
	...
	for(i=0; i<10000; i++) {
		...
		trace_android_rvh_selinux_avc_lookup((struct avc_node*)&node, node.ae.ssid, node.ae.tsid, node.ae.tclass);
	}
...
TEST(mkp_test, mkp_test_005_creds)
{
	...
	trace_android_rvh_commit_creds(cur, &cred);
