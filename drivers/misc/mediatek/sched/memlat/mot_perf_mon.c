 ...
 */
#define pr_fmt(fmt) "mot_perf_mon: " fmt
...
#include "mot_perf_mon_priv.h"

static struct mot_perf_mon_config perf_mon_config;
static struct mot_perf_mon_state perf_mon_metadata;
...
EXPORT_SYMBOL(mot_perf_mon_get_data);

static int mot_perf_mon_start(void);
static void mot_perf_mon_stop(void);
 ...
 */
static inline int read_perf_event(struct mot_event_data *event, u64 *event_total)
{
	int ret = 0;
...
EXPORT_SYMBOL(mot_perf_mon_add_client);
...
EXPORT_SYMBOL(mot_perf_mon_remove_client);
...
void mot_perf_mon_tick_update_counters(void)
{
	...
	if (cpu_data->ticks_since_update >= perf_mon_config.param_ticks_per_counter_update ||
		...
		for (perf_idx = 0; perf_idx < PERF_NUM_COMMON_EVS; perf_idx++) {
			...
			if (ev_data->raw_event_id > 0 && read_perf_event(ev_data, &total)) {
...
EXPORT_SYMBOL(mot_perf_mon_tick_update_counters);
...
*/
static void delete_event(struct mot_event_data *event)
{
	if (event->pevent) {
 ...
 * Returns:	Non-zero on error.
*/
static int init_event(struct mot_event_data *event, unsigned int cpu)
{
	...
	if (event->counter_type == PMU) {
		...
		if (IS_ERR(pevent)){
			pr_err("init_event: perf_event_create_kernel_counter on CPU #%d failed and return %ld\n", cpu, PTR_ERR(pevent));
...
static int enable_perf_events(unsigned int cpu)
{
	...
	for (perf_idx = 0; perf_idx < PERF_NUM_COMMON_EVS; perf_idx++) {
		ev_data = &cpu_data->perf_ev_data[perf_idx];
		if(ev_data->raw_event_id <= 0) {
			pr_err("PMU counter is not available on cpu[%d] because ev_data->raw_event_id[0x%x]\n", cpu, ev_data->raw_event_id);
			continue;
		}
		ret = init_event(ev_data, cpu);
		if (WARN_ON(ret)) {
			pr_err("enable_perf_events: cpu[%d], perf_idx[%u], ev_data->element_idx[%d], ev_data->counter_type[%d], ev_data->raw_event_id[0x%x]\n",
				cpu, perf_idx, ev_data->element_idx, ev_data->counter_type, ev_data->raw_event_id);
			goto err_init;
		}
	}
...
/**
 * mot_perf_mon_update_clients - Main function for updating clients.
 *
...
{
	unsigned int cpu;
	struct mot_perf_mon_client *curr_client;
	ktime_t last_update_client_ts;
	...
	if (mutex_trylock(&perf_mon_metadata.client_list_lock)) {
		...
		for_each_possible_cpu (cpu) {
			ret = mot_perf_mon_get_data(cpu, &perf_mon_metadata.client_shared_data[cpu]);
			if (ret)
...
EXPORT_SYMBOL(mot_perf_mon_update_clients);

/**
 * mot_perf_mon_cpuhp_up - re-enables perf monitoring on cpu-up.
 *
 * Input:
 * @cpu:	Whichever CPU just came back online.
*/
static int mot_perf_mon_cpuhp_up(unsigned int cpu)
{
	int ret = 0;
	int mon_active;
	unsigned long flags;
...
/**
 * mot_perf_mon_cpuhp_down - disables perf monitoring on cpu-down.
 *
 * Input:
 * @cpu:	Whichever CPU just went down.
*/
static int mot_perf_mon_cpuhp_down(unsigned int cpu)
{
	struct cpu_perf_info *cpu_data = &perf_mon_metadata.cpu_data_arr[cpu];
	mutex_lock(&cpu_data->perf_allocation_lock);
	disable_perf_events(cpu);
...
static int mot_init_perf_mon_cpuhp(void)
{
	int ret = 0;
	ret = cpuhp_setup_state_nocalls(CPUHP_AP_ONLINE_DYN, "mot_perf_mon", mot_perf_mon_cpuhp_up,
					mot_perf_mon_cpuhp_down);
	if (ret < 0)
...
/**
 * mot_perf_mon_start - Start and perf monitoring.
 *
...
/**
 * mot_perf_mon_stop - Stop perf monitoring.
 *
...
static int parse_perf_event(struct device_node *counter_type, struct mot_event_data *cpu_event,
			    enum mot_counter_type unit_id, enum mot_perf_event_idx ev_idx, char *prop_name)
{
	int ret = 0;
...
static int parse_perf_counters(struct device *dev, struct device_node *counter_type,
			       struct mot_event_data *cpu_events, enum mot_counter_type unit_id)
{
	int ret = 0;
				    ...
				    struct cpu_perf_info *cpu_data)
{
	...
	struct mot_event_data *cpu_event_data = cpu_data->perf_ev_data;
...
static int mot_perf_mon_parse_dt(struct platform_device *pdev)
{
	...
	cpu_idx = 0;
	cpu_node = NULL;
	cpus_data_np = of_get_child_by_name(dev->of_node, "mot_perf_cpu");
	if (!cpus_data_np) {
		dev_err(dev, "mot_perf_cpu invalid skipping performance monitoring.\n");
		ret = -EINVAL;
		goto err_probe;
	}
	...
	while ((cpu_node = of_get_next_child(cpus_data_np, cpu_node)) != NULL) {
		...
		dev_info(dev, "initializing cpu data on cpu[%d].\n", cpu_idx);
...
static int perf_mon_task(void *data)
{
	while (!kthread_should_stop()) {
		...
		mot_perf_mon_update_clients();
	}
	return 0;
}

int mot_perf_mon_driver_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	int ret = 0;

	ret = mot_perf_mon_parse_dt(pdev);
	if (ret)
		return ret;
	...
	perf_mon_metadata.client_shared_data = devm_kzalloc(
		dev, sizeof(struct mot_cpu_perf_data) * num_possible_cpus(), GFP_KERNEL);
	...
	ret = mot_init_perf_mon_cpuhp();
	if (ret < 0) {
		dev_err(dev, "mot_init_perf_mon_cpuhp errored with number %d\n", ret);
		goto err_cpuhp_init;
	}

	/* Start the perf monitor. */
	/*ret = mot_perf_mon_start();
	if (ret) {
		dev_err(dev, "mot_perf_mon could not stop with error code %d\n", ret);
		goto err_cpuhp_init;
	}*/
...
static struct platform_driver mot_perf_mon_platform_driver = {
	...
	.driver = {
		.name = "mot_perf_mon",
		.owner = THIS_MODULE,
		.of_match_table = mot_perf_mon_root_match,
		.suppress_bind_attrs = true,
	},
};

/* Driver initialization step for lists and locks. */
static int __init mot_perf_mon_init(void)
{
	int ret = 0;
	...
	ret = platform_driver_register(&mot_perf_mon_platform_driver);
	if (ret)
...
static void __exit mot_perf_mon_exit(void)
{
	mot_perf_mon_stop();
}

/* A module parameter for frequency of counter updates. */
static int mot_perf_mon_param_set_ticks(const char *val, const struct kernel_param *kp)
{
	unsigned int ticks = 0;

	if (kstrtoint(val, 10, &ticks)) {
		pr_err("%s: mot_perf_mon parse error", __func__);
		return -EINVAL;
	}

	if (ticks < 1 || ticks > 32) {
		pr_err("%s: mot_perf_mon invalid number of ticks", __func__);
		return -EINVAL;
	}
...
static int mot_perf_mon_param_get_ticks(char *buf, const struct kernel_param *kp)
{
	return sysfs_emit_at(buf, 0, "%u\n", perf_mon_config.param_ticks_per_counter_update);
}

static const struct kernel_param_ops param_tick = {
	.set = mot_perf_mon_param_set_ticks,
	.get = mot_perf_mon_param_get_ticks,
};

module_param_cb(mot_perf_mon_param_ticks, &param_tick, NULL, 0644);

static int mot_perf_mon_param_set_active(const char *val, const struct kernel_param *kp)
{
	bool is_active;
...
static int mot_perf_mon_param_get_active(char *buf, const struct kernel_param *kp)
{
	return sysfs_emit_at(buf, 0, "%u\n", perf_mon_metadata.is_active);
}

static const struct kernel_param_ops param_is_active = {
	.set = mot_perf_mon_param_set_active,
	.get = mot_perf_mon_param_get_active,
};

module_param_cb(perf_mon_param_on, &param_is_active, NULL, 0644);

module_init(mot_perf_mon_init);
module_exit(mot_perf_mon_exit);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Performance Monitor");
