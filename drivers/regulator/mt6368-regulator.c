...
#define MT6368_RG_STRUP_ANA_CON1_ADDR	0x0989
#define MT6368_RG_MOSCON1_EN_SHIFT		1
#define MT6368_RG_MOSCON1_EN_MASK		0x02

static int mt6368_moscon1_regulator_enable(struct regulator_dev *rdev)
{
	int ret = 0;
	unsigned int val = 0;

	ret = regmap_read(rdev->regmap, MT6368_RG_STRUP_ANA_CON1_ADDR, &val);
	pr_info("%s read ANA_CON1 val=0x%02X ret =%d\n", __func__, val, ret);
	if (ret < 0) {
		return ret;
	}

	val = val | MT6368_RG_MOSCON1_EN_MASK;
	ret = regmap_write(rdev->regmap, MT6368_RG_STRUP_ANA_CON1_ADDR, val);
...
static int mt6368_moscon1_regulator_disable(struct regulator_dev *rdev)
{
	int ret = 0;
	unsigned int val = 0;
...
static int mt6368_moscon1_regulator_is_enabled(struct regulator_dev *rdev)
{
	int ret = 0;
	unsigned int val = 0;
	...
	return (val & MT6368_RG_MOSCON1_EN_MASK) == MT6368_RG_MOSCON1_EN_MASK;
}

static const struct regulator_ops mt6368_moscon1_ops = {
	.enable = mt6368_moscon1_regulator_enable,
	.disable = mt6368_moscon1_regulator_disable,
	.is_enabled = mt6368_moscon1_regulator_is_enabled,
};

static const struct regulator_init_data mt6368_moscon1_constraints = {
	.constraints = {
...
static struct regulator_desc mt6368_moscon1_rdesc = {
	.name = "mt6368_mos_con1",
	.of_match = of_match_ptr("mt6368,mos-con1"),
	.regulators_node = of_match_ptr("regulators"),
	.ops = &mt6368_moscon1_ops,
	.owner = THIS_MODULE,
	.type = REGULATOR_VOLTAGE,
	.enable_reg = MT6368_RG_STRUP_ANA_CON1_ADDR,
	.enable_mask = MT6368_RG_MOSCON1_EN_MASK,
	.enable_val = MT6368_RG_MOSCON1_EN_MASK,
};

static int mt6368_moscon1_regulator_init(struct platform_device *pdev)
{
	...
	/*mt6368_moscon1_rdesc has default name*/
	ret = of_property_read_string(np, "regulator-name", &mt6368_moscon1_rdesc.name);
	dev_notice(&pdev->dev, "get regulator-name=%s\n", mt6368_moscon1_rdesc.name);

	moscon1_config.dev = pdev->dev.parent;
	moscon1_config.regmap = dev_get_regmap(pdev->dev.parent, NULL);
	moscon1_config.init_data = &mt6368_moscon1_constraints;
	rdev = devm_regulator_register(&pdev->dev, &mt6368_moscon1_rdesc, &moscon1_config);
	if (IS_ERR(rdev)) {
		ret = PTR_ERR(rdev);
		dev_err(&pdev->dev, "failed to register %s, ret=%d\n",
			mt6368_moscon1_rdesc.name, ret);
	}
...
static int mt6368_regulator_probe(struct platform_device *pdev)
{
	...
	mt6368_moscon1_regulator_init(pdev);
