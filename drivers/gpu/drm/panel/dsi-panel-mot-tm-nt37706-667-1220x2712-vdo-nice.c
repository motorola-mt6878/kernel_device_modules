...
#include "dsi-panel-mot-tm-nt37706-667-1220x2712-vdo-lhbm-alpha.h"
...
if((ctx->version == 3) || (ctx->version == 4)) //dvt2
{
...
if((ctx->version == 6) || (ctx->version == 5))//dvt2,pvt and mp
        {
        lcm_dcs_write_seq_static(ctx, 0x3B,0x00,0x14,0x00,0x34,0x00,0x14,0x03,0xD4,0x00,0x14,0x0B,0x14,0x00,0x14,0x10,0x84);
        lcm_dcs_write_seq_static(ctx, 0x6F,0x10);
        lcm_dcs_write_seq_static(ctx, 0x3B,0x00,0x14,0x00,0x34);
	...
	if(ctx->version == 6)
	{
		...
		lcm_dcs_write_seq_static(ctx, 0xC0,0xA7);
	}
        //120Hz
//        lcm_dcs_write_seq_static(ctx, 0x2F,0x00);
        lcm_dcs_write_seq_static(ctx, 0x81,0x01,0x19);
...
static char bl_tb_aod_apl[] = {0xA9,0x01,0x00,0x5F,0x00,0x01,0x00,0x00,0x01,0x00,0x51,0x00,0x01,0x3E,0x80,0x01,0x00,0x51,0x04,0x05,0x03,0x34};// apl:bit7  normal_bl: bit 13~14 aod_bl: 20~21
...
//	if ((ctx->version == 1) || (ctx->version == 2))
	...
	if ((ctx->version == 5) || (ctx->version == 6))
	{
		if(fps == 90)
			pTable->para_list[28] = 0x6E;
		if(fps == 60)
			pTable->para_list[28] = 0xAE;
	}
