...
#include "include/dsi-panel-mot-csot-nt37706-667-1220x2712-dphy-vdo-120hz-lhbm-alpha_v4.h"
...
static struct mtk_panel_para_table panel_lhbm_on_v4[] = {
...
static struct mtk_panel_para_table panel_lhbm_off_v4[] = {
	{22, {0xA9, 0x01, 0x00, 0x5F, 0x00, 0x01, 0x00, 0x00, 0x01, 0x00, 0x51, 0x00, 0x01, 0x3E, 0x80, 0x01, 0x00, 0x51, 0x04, 0x05, 0x05, 0x54}},// apl:bit7  normal_bl: bit 13~14 aod_bl: 20~21
...
static struct mtk_panel_para_table panel_lhbm_off_dc_v4[] = {
	{22, {0xA9, 0x01, 0x00, 0x5F, 0x00, 0x01, 0x00, 0x00, 0x01, 0x00, 0x51, 0x00, 0x01, 0x3E, 0x80, 0x01, 0x00, 0x51, 0x04, 0x05, 0x05, 0x54}},// apl:bit7  normal_bl: bit 13~14 aod_bl: 20~21
	{48, {0xA9, 0x02, 0x00, 0xC0, 0x2E, 0x2F, 0x32, 0x60, 0x02, 0x00, 0xC0, 0x30, 0x33, 0x04, 0x57, 0x00, 0x88, 0x02, 0x00, 0xC7, 0x01,
	      0x01, 0x01, 0x02, 0x08, 0xC1, 0x00, 0x01, 0x8E, 0xFF, 0x01, 0x00, 0x8B, 0x00, 0x00, 0x00, 0x01, 0x00, 0x87, 0x00, 0x00, 0x20,
...
static void set_lhbm_alpha(struct lcm *ctx, unsigned int bl_level, struct mtk_panel_para_table *panel_lhbm_on)
{
	...
	if (ctx->version < 4)
		...
		alpha = lhbm_alpha_v4[lhbm_alpha_index];
...
static int panel_lhbm_set_cmdq(struct lcm *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t on, uint32_t bl_level, uint32_t fps)
{
	...
	if (on)
	{
		if (ctx->version < 4){
			...
			para_count = sizeof(panel_lhbm_on_v4) / sizeof(struct mtk_panel_para_table);
			pTable = panel_lhbm_on_v4;
		}
	...
	else
	{
		...
		else
		{
			if (atomic_read(&ctx->dc_mode)) {
				para_count = sizeof(panel_lhbm_off_dc_v4) / sizeof(struct mtk_panel_para_table);
				pTable = panel_lhbm_off_dc_v4;
			} else {
				para_count = sizeof(panel_lhbm_off_v4) / sizeof(struct mtk_panel_para_table);
				pTable = panel_lhbm_off_v4;
			}
		}
...
static struct mtk_panel_para_table panel_dc_off_v4[] = {
	{2, {0x6f, 0x01}},
	{2, {0x8b, 0x00}},
	{6, {0xf0, 0x55, 0xaa, 0x52, 0x08, 0x00}},
	{27, {0xB3, 0x00, 0x04, 0x04, 0x5C, 0x04, 0x5C, 0x06, 0xCC, 0x06, 0xCC, 0x09, 0xD0, 0x09, 0xD0, 0x0D, 0xC8, 0x0D, 0xC8, 0x10, 0xAF, 0x10, 0xAF, 0x10, 0xB0, 0x10, 0xB0}},
...
static struct mtk_panel_para_table panel_dc_on_v4[] = {
	{2, {0x6f, 0x01}},
	{2, {0x8b, 0x81}},
	{6, {0xf0, 0x55, 0xaa, 0x52, 0x08, 0x00}},
	{27, {0xB3, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0xB0}},
...
static int pane_dc_set_cmdq(struct lcm *ctx, void *dsi, dcs_grp_write_gce cb, void *handle, uint32_t dc_state)
{
	...
	if (dc_state) {
		if (ctx->version < 4)
		{
			...
			para_count = sizeof(panel_dc_on_v4) / sizeof(struct mtk_panel_para_table);
			pTable = panel_dc_on_v4;
		}
	} else {
		if (ctx->version < 4)
		{
			...
			para_count = sizeof(panel_dc_off_v4) / sizeof(struct mtk_panel_para_table);
			pTable = panel_dc_off_v4;
		}
	}
	cb(dsi, handle, pTable, para_count);
