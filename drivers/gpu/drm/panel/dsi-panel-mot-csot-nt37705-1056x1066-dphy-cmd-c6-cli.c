...
static enum panel_version panel_ver = PANEL_V1;
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx, 0xCC, 0x40, 0x54, 0x5A, 0x60, 0x66, 0x6A, 0x6C, 0x6C, 0x68, 0x62, 0x5B, 0x53, 0x4D, 0x48, 0x44, 0x41, 0x40, 0x50, 0x60, 0x70, 0x58, 0x68, 0x78, 0x00);
	lcm_dcs_write_seq_static(ctx, 0xCD, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x45, 0x49, 0x4C, 0x4B, 0x48, 0x44, 0x41, 0x40);
	lcm_dcs_write_seq_static(ctx, 0xCE, 0x00, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40);
	lcm_dcs_write_seq_static(ctx, 0xCF, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40);
	lcm_dcs_write_seq_static(ctx, 0xD0, 0x01, 0x00, 0xFF, 0x77);
	lcm_dcs_write_seq_static(ctx, 0xD1, 0x3D, 0x34, 0x3D, 0x47, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x50, 0x47, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x0C, 0x03, 0x09, 0x0F, 0x68, 0x88, 0x00);
	lcm_dcs_write_seq_static(ctx, 0xD2, 0x00, 0x00, 0x40, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80);
	lcm_dcs_write_seq_static(ctx, 0xD4, 0x00, 0x76, 0x76, 0x05, 0x0D, 0x14, 0x0F, 0x18, 0x1F, 0x05, 0x0D, 0x14, 0x03, 0x07, 0x0A, 0xA0, 0x55, 0xFF, 0x39, 0x44, 0x12, 0x00, 0x00, 0x00);
	lcm_dcs_write_seq_static(ctx, 0xD7, 0x00, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40);
	...
	//3D LUT dimming off for C6 panel
	lcm_dcs_write_seq_static(ctx, 0xF0, 0x55, 0xAA, 0x52, 0x08, 0x00);
	lcm_dcs_write_seq_static(ctx, 0x6F, 0x1A);
...
static struct mtk_panel_params ext_params_90hz = {
	...
	.panel_ver = 1,
	.panel_name = "csot_nt37705_1056_1066_c6",
...
static struct mtk_panel_para_table panel_dc_off[] = {
	...
	{31, {0xB3, 0x00, 0x01, 0x01, 0x58, 0x01, 0x58, 0x02, 0x53, 0x02, 0x53, 0x03, 0x09, 0x03, 0x09, 0x04, 0x00, 0x04, 0x00, 0x04, 0xBE, 0x04, 0xBE, 0x05, 0x22, 0x05, 0x22, 0x05, 0x2B, 0x05, 0x2B}},
	{2, {0xB2, 0x01}},
	{6, {0xF0, 0x55, 0xAA, 0x52, 0x08, 0x02}},
	{2, {0xCC, 0x10}},
};
...
static struct mtk_panel_para_table panel_ce_off[] = {
	{5, {0xFF, 0xAA, 0x55, 0xA5, 0x81}},
	{2, {0x6F, 0x0E}},
	{2, {0xF5, 0x28}},
			      ...
			      dcs_grp_write_gce cb, void *handle, bool en)
{
	...
	if (atomic_read(&ctx->ce_mode) == en) {
		return 0;
	}

	if(en){
		para_count = sizeof(panel_ce_on) / sizeof(struct mtk_panel_para_table);
		pTable = panel_ce_on;
	} else {
		para_count = sizeof(panel_ce_off) / sizeof(struct mtk_panel_para_table);
		pTable = panel_ce_off;
	}

	cb(dsi, handle, pTable, para_count);

	atomic_set(&ctx->ce_mode, en);

	pr_info("%s: set panel ce to %d success\n", __func__, en);
	return 0;
}
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "csot_nt37705_1056_1066_c6",
		.owner = THIS_MODULE,
